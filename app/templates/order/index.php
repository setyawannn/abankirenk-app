...
<div class="space-y-6">
  <div class="rounded-lg flex flex-col md:flex-row justify-between items-center gap-4">
    ...
    {{-- Tombol Tambah --}}
    <a href="{{ url('/order/create') }}" class="btn-df w-full md:w-auto flex-shrink-0">
  ...
  $(document).ready(function() {
    ...
    function renderTable(data) {
      ...
      data.forEach((order, index) => {
        // const editUrl = `{{ url('/order') }}/${order.id_order_produksi}/edit`;

        const detailUrl = `{{ url('/order') }}/${order.id_order_produksi}/detail`;
